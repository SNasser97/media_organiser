...
<body>
	...
	if(isset($_GET['id'])) { //if table id is set
		...
		while($row = mysqli_fetch_assoc($get_query)) { //loop through associative array and return media to be played
			...
			echo "<embed src='$url' height='480' width='720'><br><a class='btn btn-primary' href='php_scripts/change.php?=$id'>Change Name</a><a class='btn btn-primary' href='php_scripts/remove.php?=$id'>Remove</a>"; // Add edit tag
		}
	} else {
